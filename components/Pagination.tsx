...
const Pagination = ({ currentPage, totalPages, onPageChange }: PaginationProps) => {
  const renderPageNumbers = () => {
    ...
    if (currentPage > 3) {
      pages.push(
        <Button
          ...
          className={`px-3 py-3 w-8 h-8 rounded-md text-xs font-semibold ${
            currentPage === 1 ? 'bg-gradient-to-r from-[#00b5f1] to-[#00e0ff] text-white' : 'bg-slate-100 text-primary'
          }`}
        >
          1
        </Button>
      ...
      // Thêm dấu ...
      if (currentPage > 4) {
        pages.push(<span key="ellipsis1" className="px-3 w-8 h-8">...</span>);
      }
    }
      ...
      pages.push(
        <Button
          ...
          className={`px-3 py-3 w-8 h-8 rounded-sm ${
            currentPage === i ? 'bg-gradient-to-r from-[#00b5f1] to-[#00e0ff] text-white' : 'bg-slate-100 text-primary'
          }`}
        >
          {i}
        </Button>
    ...
    if (currentPage < totalPages - 2) {
      ...
      pages.push(
        <Button
          ...
          className={`px-3 py-3 w-8 h-8 rounded-sm ${
            currentPage === totalPages ? 'bg-gradient-to-r from-[#00b5f1] to-[#00e0ff] text-white' : 'bg-slate-100 text-primary'
          }`}
        >
          {totalPages}
        </Button>
