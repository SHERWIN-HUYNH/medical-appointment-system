...
import { DateSelectArg, DatesSetArg, DayCellContentArg, EventClickArg, EventSourceInput } from "@fullcalendar/core/index.js";
import interactionPlugin, { DateClickArg, Draggable, DropArg } from '@fullcalendar/interaction'
import { useState, useEffect, Fragment, useRef } from "react";
import timeGridPlugin from '@fullcalendar/timegrid'
import clsx from "clsx";
import { Button } from "@/components/ui/button";
import { fetchEventsFromApi, formatTimeSlot, transformApiEventData } from "@/helpers/formatTimeSlots";
...
const DoctorSchedule = ({doctorId} : DoctorScheduleProps) => {
  ...
  const handleEventClick = (clickInfo: EventClickArg) => {
    ...
    const isConfirmed = window.confirm(`Do you want to delete the timeslot ${eventTitle}?`);
  ...
  const handleEventDrop = (dropInfo: EventDropArg) => {
    ...
    setAllEvents((prevEvents) => {
      return prevEvents.filter((existingEvent) => existingEvent.id !== eventId)
        .concat(newEvent);  
    });
    event.remove();
  ...
  return (
    ...
    </div><Button onClick={saveSelectedTimeSlots} className=' text-white hover:bg-primary/50'>Save Timeslots</Button>
    </div>
  );
};
